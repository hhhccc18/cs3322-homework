...
public class JoinOptimizer {
	...
	public static int estimateTableJoinCardinality(Predicate.Op joinOp,
		...
		switch (joinOp) {
			...
			case GREATER_THAN_OR_EQ:
				int field1 = stats.get(table1Alias).getIndex(field1PureName),
						field2 = stats.get(table2Alias).getIndex(field2PureName);
				if (field1 == -1 || field2 == -1)
					card = card1 * card2 / 2;
				else {
					double min = stats.get(table1Alias).estimateSelectivity(field1,
							joinOp, new IntField(stats.get(table2Alias).getMin(field2)));
					double max = stats.get(table1Alias).estimateSelectivity(field1,
							joinOp, new IntField(stats.get(table2Alias).getMax(field2)));
					card = (stats.get(table1Alias).estimateTableCardinality(min)
							+ stats.get(table2Alias).estimateTableCardinality(max)) * card2 / 2;
				}
				break;
	 ...
	 */
	@Deprecated
	public <T> Stream<Set<T>> DEPRECATED_numerateSubsets(Vector<T> v, int size) {
		int n = v.size();
		return IntStream.range(0, 1 << n)
				.filter(i -> Integer.bitCount(i) == size)
				.mapToObj(i -> IntStream.range(0, n - 1)
						.filter(j -> (i >> j & 1) != 0)
						.mapToObj(v::get)
						.collect(Collectors.toSet()));
	}
	
	private Set<LogicalJoinNode> getSubset(int s) {
		return IntStream.range(0, this.joins.size())
				.filter(i -> (s >> i & 1) != 0)
				.mapToObj(i -> this.joins.get(i))
				.collect(Collectors.toSet());
	}
...
//		for (int k = 1; k <= this.joins.size(); k++) {
//			for (Set<LogicalJoinNode> subset : this.enumerateSubsets(this.joins, k)) {
//				CostCard best = new CostCard();
//				best.cost = Double.MAX_VALUE;
//
//				for (LogicalJoinNode o : subset) {
//					CostCard cur = this.computeCostAndCardOfSubplan(stats,
//							filterSelectivities, o, subset, Double.MAX_VALUE, pc);
//
//					if (cur != null && cur.cost < best.cost)
//						best = cur;
//				}
//
...
//		}
		...
		for (int s = 0; s < (1 << n); s++) {
			...
			for (int i = 0; i < n; i++) {
				...
				CostCard cur = this.computeCostAndCardOfSubplan(stats,
						filterSelectivities, i, s, Double.MAX_VALUE, pc);
...
//	@SuppressWarnings("unchecked")
	private CostCard computeCostAndCardOfSubplan(
			...
			/*LogicalJoinNode*/ int joinToRemove, /*Set<LogicalJoinNode> joinSet*/ int joinSetBitmask,
			double bestCostSoFar, PlanCache pc) throws ParsingException {
		
		int j = joinToRemove;
		LogicalJoinNode jNode = this.joins.get(j);
		
		Vector<LogicalJoinNode> prevBest;
		
		if (this.p.getTableId(jNode.t1Alias) == null)
			throw new ParsingException("Unknown table " + jNode.t1Alias);
		if (this.p.getTableId(jNode.t2Alias) == null)
			throw new ParsingException("Unknown table " + jNode.t2Alias);
		
		String table1Name = Database.getCatalog().getTableName(
				this.p.getTableId(jNode.t1Alias));
		String table2Name = Database.getCatalog().getTableName(
				this.p.getTableId(jNode.t2Alias));
		String table1Alias = jNode.t1Alias;
		String table2Alias = jNode.t2Alias;
		
//		Set<LogicalJoinNode> news = (Set<LogicalJoinNode>) ((HashSet<LogicalJoinNode>) joinSet).clone();
//		Set<LogicalJoinNode> news = this.getSubset(joinSetBitmask);
		int news = joinSetBitmask;
...
//		news.remove(j);
		...
		if (news == 0) { // base case -- both are base relations
			...
			t1card = stats.get(table1Name).estimateTableCardinality(
					filterSelectivities.get(jNode.t1Alias));
			leftPkey = isPkey(jNode.t1Alias, jNode.f1PureName);
			
			t2cost = table2Alias == null ? 0 : stats.get(table2Name)
					.estimateScanCost();
			t2card = table2Alias == null ? 0 : stats.get(table2Name)
					.estimateTableCardinality(
							filterSelectivities.get(jNode.t2Alias));
			...
			if (doesJoin(prevBest, table1Alias)) { // j.t1 is in prevBest
				...
				t2cost = jNode.t2Alias == null ? 0 : stats.get(table2Name)
						.estimateScanCost();
				t2card = jNode.t2Alias == null ? 0 : stats.get(table2Name)
						.estimateTableCardinality(
								filterSelectivities.get(jNode.t2Alias));
				rightPkey = jNode.t2Alias != null && isPkey(jNode.t2Alias,
						jNode.f2PureName);
			} else if (doesJoin(prevBest, jNode.t2Alias)) { // j.t2 is in prevbest
				// (both
				// shouldn't be)
		...
		double cost1 = estimateJoinCost(jNode, t1card, t2card, t1cost, t2cost);
		
		LogicalJoinNode j2 = jNode.swapInnerOuter();
		...
		cc.card = estimateJoinCardinality(jNode, t1card, t2card, leftPkey,
				rightPkey, stats);
		cc.cost = cost1;
		
//		cc.plan = (Vector<LogicalJoinNode>) prevBest.clone();
		cc.plan = new Vector<>();
		cc.plan.addAll(prevBest);
		
		cc.plan.addElement(jNode); // prevbest is left -- add new join to end
		return cc;
	}
...
//			pathSoFar.add(j);
			...
			LogicalJoinNode jNode = js.get(j);
			...
			m.put(jNode.t1Alias, root);
			
			n = m.get(jNode.t2Alias);
			if (n == null) { // never seen this table before
				
				n = new DefaultMutableTreeNode(
						jNode.t2Alias == null ? "Subplan"
								: (jNode.t2Alias
								+ " (Cost = "
